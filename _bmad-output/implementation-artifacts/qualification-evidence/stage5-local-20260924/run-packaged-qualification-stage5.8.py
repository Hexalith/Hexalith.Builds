#!/usr/bin/env python3
"""Run and hash-bind the Stage 5 (EventStore 3.108.1 requalification, 0.0.0-stage5.8) packaged-tool qualification through installed commands only.
...
def main() -> int:
    ...
    dirty_lines = subprocess.run(["git", "-C", str(ROOT), "status", "--porcelain", "--untracked-files=all"], capture_output=True, text=True, check=True).stdout.splitlines()
    ...
    candidate = {
        ...
        "platformPins": {"eventStoreVersion": "3.108.1", "daprRuntimeVersion": "1.18.2", "daprSdkVersion": "1.18.8", "frontComposerVersion": "4.5.0"},
    ...
    report = {
        ...
        "version": VERSION,
        "sourceRevision": revision,
        "repositoryDirty": dirty_lines != [],
        "repositoryDirtyLines": dirty_lines,
