...
## Deferred from: code review of 6-1-p0-deliver-g4-persisted-runner-and-evidence-tooling (Stage 5, 2026-09-25)

- **The fixture-proof self-test is not wired into CI.** `Tools/test-g4-tool-package-artifact-validator.ps1`, which should also gain a `.gitattributes` CRLF case, is not run by `.github/workflows/ci.yml`. **Resolved 2026-09-25 (SR-W1):** `test-g4-tool-package-contract-gate.ps1`, which CI already runs, now runs this self-test. The G-6-bound `ci.yml` was not edited.
- **The packaged native-report live lane is opt-in.** `PackagedPersistedProfileTests` skips unless `HEXALITH_G4_LIVE=1`, so VSTest/MTP report binding through the installed tool is proven only by manual campaigns (AC11). Deferred: this needs live-capable CI infrastructure (Stages 6–7).
- **Failing native invocations bind no counts or report.** `NativeTestExecutor.Evaluate` returns no report on any nonpassing path, so failed evidence has `testCounts.reported=false` and no report hash (AC7). Deferred: this is a recorded Stage 5 residual. Resolve it with the native-report retention decision (SR-D2) or before the Stage 7 acceptance record.
